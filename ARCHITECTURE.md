...
#### Show-details guarantee (`scripts/lib/show-details.js`)

The writer is told to weave the date, venue and a `Get Tickets` link into the narrative; the fact-check pass then removed any sentence not backed by the comedian research (the show logistics never are) and the polish pass cut the weakest paragraph to fit 400 words. On 2026-09-22 only 4 of the 20 live spotlights named the show date **and** linked tickets in the body. Two layers fix it: the fact-check and polish prompts now receive a `SHOW FACTS` block (comedian, venue, date, time, price, ticket URL) marked as verified-by-us with an explicit keep rule, and `ensureShowDetails()` runs after sanitizing as a deterministic backstop, inserting `<p class="show-details"><strong>Name</strong> plays Venue on Date. Get Tickets.</p>` before the post footer whenever the body still lacks the date or a ticket link. `scripts/backfill-show-details.js` (workflow `backfill-show-details.yml`, dry-run by default) applies the same check to posts already live, touching only future-dated ones.

#### Source-link verification (`scripts/lib/link-check.js`)

The writer cites URLs from the research but invents one now and then (`youtube.com/watch?v=xyz123` shipped in the Martin Amini spotlight, 2026-09-01), and the fact-checker "preserves all hyperlinks to real source URLs" with no way to know which are real. After sanitizing, `verifyOutboundLinks()` fetches every external link in the body: YouTube URLs go through the oEmbed endpoint (a bad video id still renders a 200 page, oEmbed answers 400/404), everything else is a short GET following redirects. Dead links (404/410, or oEmbed rejecting the id) are **unwrapped**, keeping the sentence text and dropping the citation. 403/429/5xx/timeouts are inconclusive and left alone, as are internal comedyhouston.com links and ticket links. `backfill-show-details.js` runs the same pass on live posts (`VERIFY_LINKS=0` to skip).

#### WordPress publishing — preflight & error logging

Before any OpenAI call, `wpPreflight()` calls `GET /wp-json/wp/v2/users/me?context=edit`. A 403, 408, 429, 5xx or network error is retried up to `WP_PREFLIGHT_ATTEMPTS` (4) times with `WP_PREFLIGHT_RETRY_MS` (35s) between tries: Hostinger's edge ("hcdn") intermittently serves the GitHub runner a 403 "Checking your browser" page instead of the API (2026-09-07, 2026-09-22), and it clears within minutes. A 401 or 400 is deterministic and fails on the first try. If the preflight never passes the run exits 1 **before any OpenAI spend** and the workflow's failure alert fires; re-dispatch once the host is reachable. `WP_PREFLIGHT_SOFT=1` restores the old carry-on behaviour (graphics + captions + manifest without WordPress) for a deliberate graphics-only run. It used to carry on by default: the 2026-09-22 backfill paid for five posts it could not publish and committed a manifest with no `wpLink` for the IG poster.
...
| **Gate** | `just-announced.js` flags "notable" listings via `lib/announce-rules.js` + `config/announce-venues.json`: the show must be ≥ 14 days out, the title must not look like a showcase / open mic / themed night, the same title must not repeat at that venue across 3+ weeks (series detection; a Thu–Sat headliner run is one week), and the venue must be a **big room** (arenas, stadiums, theaters, music halls) or a **headliner room** (Improv, Punch Line, Riot main room, MATCH), or an unlisted venue whose face-value floor `price_min` ≥ $45. `price_max` is never consulted (VIP tables inflated The Den's $19 showcases to $227 and flagged them, 2026-09-08). Every feed entry is re-scored on each events run. Then gpt-5.6-terra answers: is this a headline show by a named, recognizable comedian? Only `qualifies` with confidence ≥ 0.75 goes out, at most `ANNOUNCE_MAX` (1) per day, never the same performer twice in 45 days. Skips are recorded in `config/announced-posted.json` so a listing is judged once. |
...
The local launchd job (`~/showList/growth-loop/daily-announce.sh`) still renders preview cards and notifies; it is no longer the posting path. It has no LLM gate, so it renders exactly what the rules flag; tighten `config/announce-venues.json` rather than the script if the previews drift.
